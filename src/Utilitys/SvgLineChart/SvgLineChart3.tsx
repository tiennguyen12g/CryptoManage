...
export default function SvgLineChart3({dataChart}: SvgLinChartProps) {
  const {colors} = useTheme();
  const [chartFrame, setChartFrame] = useState<string>('7');
  const [currentSvgBox, setCurrentSvgBox] = useState();
  ...
  const maxX = width - textWidth - 10 - 5 - 30; // 30 is padding screen; 10 svgMarginLeftInsideScrollView; 5 is marginLeft of scrollView that contain Svg.
  ...
  const totalPointCanSeeOnAScreen =
    totalPointDatas > 10 ? 10 : totalPointDatas + 1;
  ...
  const numberOfSvgWindowNeeds =
    Math.floor(totalPointDatas / totalPointCanSeeOnAScreen) + 1;
